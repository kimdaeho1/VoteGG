    ...
    async joinSession() {
        ...
        session.on('connectionCreated', (event) => {
            if (event.connection.connectionId !== session.connection.connectionId) {
                const currentTranscriptions = useTranscriptionStore.getState().transcriptionHistory;
                session.signal({
                    data: JSON.stringify({
                        ...
                        transcriptionHistory: currentTranscriptions // 추가
                    }),
                    to: [event.connection],
                    type: 'userList',
                });
        ...
        session.on('signal:userList', (event) => {
            ...
            this.setState((prevState) => {
                ...
                if (data.transcriptionHistory && data.transcriptionHistory.length > 0) {
                    const addTranscription = useTranscriptionStore.getState().addTranscription;
                    data.transcriptionHistory.forEach(transcription => {
                        // 중복 체크
                ...
                return {
                    ...
                    transcriptionHistory: [...prevState.transcriptionHistory, ...(data.transcriptionHistory || [])]
                };
            });
        });
        ...
        session.on('signal:transcription', (event) => {
            ...
            if (!isDuplicate) {
                addTranscription(transcriptionData);
                this.setState(prevState => ({
                    transcriptionHistory: [...prevState.transcriptionHistory, transcriptionData]
                }));
            }
        });
